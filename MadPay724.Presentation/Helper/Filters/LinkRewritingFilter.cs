...
using MadPay724.Common.Helper.MediaTypes;
        ...
        private static void ReWriteAllLinks(object model, LinkRewriter rewriter)
        {
          ...
          .Where(p => p.CanRead).ToArray();
            ...
            foreach (var linkPeoperty in linkProperties)
            {
                ...
                {
                    allProperties.SingleOrDefault(p => p.Name == nameof(BaseDto.Href))?.SetValue(model, rewritten.Href);
                    allProperties.SingleOrDefault(p => p.Name == nameof(BaseDto.Method))?.SetValue(model, rewritten.Method);
                    allProperties.SingleOrDefault(p => p.Name == nameof(BaseDto.Relations))?.SetValue(model, rewritten.Relations);
                }
            }
